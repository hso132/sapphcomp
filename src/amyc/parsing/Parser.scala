...
object Parser extends Pipeline[Stream[Token], Program] {
  ...
  lazy val amyGrammarLL1 = Grammar('Program, List[Rules[Token]](
    ...
    'OrList ::= OR() ~ 'RestOr | epsilon(),
    'RestOr ::= 'AndExpr ~ 'OrList,
    'AndExpr ::= 'EqExpr ~ 'AndList,
    'AndList ::= AND() ~ 'RestAnd | epsilon(),
    'RestAnd ::= 'EqExpr ~ 'AndList,
    'EqExpr ::= 'CompExpr ~ 'EqList,
    'EqList ::= EQUALS() ~ 'RestEq | epsilon(),
    'RestEq ::= 'CompExpr ~ 'EqList,
    ...
    'Pattern ::= 'Id ~ 'CaseClassOrId | 'Literal | LPAREN() ~ RPAREN() | UNDERSCORE(),
